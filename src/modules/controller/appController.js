...
const appController = {
  ...
  init() {
    ...
    const hasTasks = this.projects.some(project => project.tasks && project.tasks.length > 0);
    ...
    if (!hasTasks) {
      ...
      this.projects[0].tasks = [exampleTasks[0], exampleTasks[1], exampleTasks[2], exampleTasks[3]]; // All Tasks in 'All' Project
      ...
      console.log('At least one project has tasks, updating "All" project tasks');
  ...
  editTask(projectIndex, taskIndex, updates) {
    ...
    if (projectIndex !== 0) {
      ...
      const taskInAll = allTasks.find(t => t.title === task.title && t.projectId === task.projectId);
      if (taskInAll) {
  ...
  deleteTask(projectIndex, taskIndex) {
    ...
    if (projectIndex !== 0) {
      this.projects[0].tasks = this.projects[0].tasks.filter(t => !(t.title === task.title && t.projectId === task.projectId));
    }
    this.save();
  },
};
